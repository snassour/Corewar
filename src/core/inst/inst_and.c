...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/07 10:20:42 by imhaimou          #+#    #+#             */
/*   Updated: 2018/10/07 19:00:40 by piquerue         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline int	set_param(int32_t *param, uint32_t type, \
	t_vproc *proc, uint8_t *mem)
{
	if (type == TYPE_REG)
	{
...
int					inst_and(t_corewar *core, t_warrior *warr, t_vproc *proc)
{
	uint8_t			*mem;
	int32_t			param[3];
	int32_t			type[3];
	...
	if (set_param(&(param[0]), type[0], proc, &(mem[2])) == ERROR || \
		...
		== ERROR || is_reg_regular(mem[2 + p_s[0] + p_s[1]]) == ERROR)
		return (inst_set_pc_error(core, proc, 2, p_s[0] + p_s[1] + p_s[2]));
	if (is_reg_regular(mem[2 + p_s[0] + p_s[1]]) == ERROR)
		return (inst_set_pc_error(core, proc, 2, p_s[0] + p_s[1] + p_s[2]));
