...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/10/07 10:59:38 by imhaimou          #+#    #+#             */
/*   Updated: 2018/10/07 23:07:05 by suddin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int					inst_lfork(t_corewar *core, t_warrior *warr, t_vproc *proc)
{
	uint8_t			*mem;
	int32_t			param;
	t_vproc			*nproc;
	...
	param = bytes_to_int(&(mem[1]), VM_IND_SIZE);
	nproc = NULL;
	...
	set_pc(core, nproc, nproc->reg[REG_PC], FORK_PROC_FLAG);
